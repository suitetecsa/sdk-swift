extension User: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    let serviceLists = try container.nestedContainer(keyedBy: ServicesKeys.self, forKey: .services)
    mailServices =
      try serviceLists.decodeIfPresent([String: MailService].self, forKey: .mailServices)?
      .compactMap { _, service in service } ?? []
    navServices =
      try serviceLists.decodeIfPresent([String: NavService].self, forKey: .navServices)?.compactMap
    { _, service in service } ?? []
    mobileServices =
      try serviceLists.decodeIfPresent([String: MobileService].self, forKey: .mobileServices)?
      .compactMap { _, service in service } ?? []
    fixedTelephony =
      try serviceLists.decodeIfPresent([String: FixedTelephony].self, forKey: .fixedTelephony)?
      .compactMap { _, service in service } ?? []
  }
